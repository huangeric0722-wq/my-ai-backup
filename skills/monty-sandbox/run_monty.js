const fs = require('fs');
const { Monty, MontySyntaxError, MontyRuntimeError, MontyTypingError } = require('@pydantic/monty');

// Parse command line arguments
// Usage: node run_monty.js <script_path> [input_json]
...
try {
  ...
  if (error instanceof MontySyntaxError) {
    errorResponse.type = "SyntaxError";
  } else if (error instanceof MontyRuntimeError) {
    errorResponse.type = "RuntimeError";
    errorResponse.traceback = error.traceback ? error.traceback() : "No traceback available";
  } else if (error instanceof MontyTypingError) {
    errorResponse.type = "TypeError";
    errorResponse.diagnostics = error.displayDiagnostics ? error.displayDiagnostics() : "No diagnostics available";
  } else {
